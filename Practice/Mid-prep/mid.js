$(function () {
  $("#j").append(
    `<button class="btn btn-danger m-3" id="sort">Sort List A</button>`
  );
  $("#j").append(
    `<button class="btn btn-danger m-3" id="sort1">Sort List B</button>`
  );
  $("#j").append(
    `<button class="btn btn-danger m-3" id="bg">Change Background</button>`
  );
  ...
  // fetchData()
  $("#d").append(
    '<Button class="btn btn-primary" id="ad">CHange color</Button>'
  );
  ...
  $.ajax({
    ...
    success: function (response) {
      for (var x = 0; x < response.length; x++) {
        $("#body")
          .append(`<tr id="${response[x]._id}"><td>${response[x]._id}</td>
